...
# Set octave to play
       ...
       freq=`python -c "print $freq * 2.0 ** $oct"`

# Set note length     
     bpm=`python -c "print 60.0 / $qtr * 1000.0"`
     case $len in
         [Ee]) length=`python -c "print $bpm / 2 "` ;;
         [Qq]) length=`python -c "print $bpm * 1"` ;;
         [Hh]) length=`python -c "print $bpm * 2"` ;;
         [Ww]) length=`python -c "print $bpm * 4"` ;;
     esac
