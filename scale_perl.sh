...
# Set octave to play
       ...
       freq=`perl -e "print $freq * 2 ** $oct"`

# Set note length     
     bpm=`perl -e "print 60 / $qtr * 1000"`
     case $len in
         [Ee]) length=`perl -e "print $bpm / 2 "` ;;
         [Qq]) length=`perl -e "print $bpm * 1"` ;;
         [Hh]) length=`perl -e "print $bpm * 2"` ;;
         [Ww]) length=`perl -e "print $bpm * 4"` ;;
     esac
