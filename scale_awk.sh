...
# Set octave to play
       ...
       freq=`echo - | awk "{print $freq * 2 ^ $oct}"`

# Set note length     
     bpm=`echo - | awk "{print 60 / $qtr * 1000}"`
     case $len in
         [Ee]) length=`echo - | awk "{print $bpm / 2}"` ;;
         [Qq]) length=`echo - | awk "{print $bpm * 1}"` ;;
         [Hh]) length=`echo - | awk "{print $bpm * 2}"` ;;
         [Ww]) length=`echo - | awk "{print $bpm * 4}"` ;;
     esac
