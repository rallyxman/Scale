...
# Set octave to play
       ...
       freq=`/usr/bin/ruby -e "print $freq * 2.0 ** $oct"`

# Set note length     
     bpm=`/usr/bin/ruby -e "print 60.0 / $qtr * 1000.0"`
     case $len in
         [Ee]) length=`/usr/bin/ruby -e "print $bpm / 2 "` ;;
         [Qq]) length=`/usr/bin/ruby -e "print $bpm * 1"` ;;
         [Hh]) length=`/usr/bin/ruby -e "print $bpm * 2"` ;;
         [Ww]) length=`/usr/bin/ruby -e "print $bpm * 4"` ;;
     esac
