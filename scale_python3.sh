...
# Set octave to play
       ...
       freq=`python3 -c "print($freq * 2.0 ** $oct)"`

# Set note length     
     bpm=`python3 -c "print(60.0 / $qtr * 1000.0)"`
     case $len in
         [Ee]) length=`python3 -c "print($bpm / 2)"` ;;
         [Qq]) length=`python3 -c "print($bpm * 1)"` ;;
         [Hh]) length=`python3 -c "print($bpm * 2)"` ;;
         [Ww]) length=`python3 -c "print($bpm * 4)"` ;;
     esac
